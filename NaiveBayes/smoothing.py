...
for word in vocabulary:
    if word in text_c:
        p_c[word] = float(text_c[word] + q)/(n_c + q * len(vocabulary))
    else:
        p_c[word] = float(q)/(n_c + q * len(vocabulary))
    if word in text_l:
        p_l[word] = float(text_l[word] + q)/(n_l + q * len(vocabulary))
    else:
        p_l[word] = float(q)/(n_l + q * len(vocabulary))
